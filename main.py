...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', '--performance', dest='performance', action='store_true',  help='Run performance tools (YSlow). Because the test is slow and resource intensive, this is best done after all other metrics are passing for an audit')
    parser.set_defaults(performance=False)
    parser.add_argument('-nr', '--no-report', dest='report', action='store_false', help='Prevent the generate of CSVs in the report directory. Ideal if you are using the web app')
    parser.set_defaults(report=True)
    parser.add_argument('-nc', '--no-crawl', dest='crawl', action='store_false', help='Prevent a crawl. Ideal for generating reports based on existing crawls')
